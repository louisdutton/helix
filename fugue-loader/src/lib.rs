...
use fugue_stdx::{env::current_working_dir, path};
...
/// 3. `FUGUE_RUNTIME` (if environment variable is set)
/// 4. `FUGUE_DEFAULT_RUNTIME` (if environment variable is set *at build time*)
/// 5. subdirectory of path to fugue executable (always included)
///
...
fn prioritize_runtime_dirs() -> Vec<PathBuf> {
    ...
    if let Ok(dir) = std::env::var("FUGUE_RUNTIME") {
    ...
    if let Some(dir) = std::option_env!("FUGUE_DEFAULT_RUNTIME") {
...
pub fn workspace_config_file() -> PathBuf {
    find_workspace().0.join(".fugue").join("config.toml")
}
...
/// and returns the first directory that contains either `.git`, `.svn`, `.jj` or `.fugue`.

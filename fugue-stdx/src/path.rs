...
///
/// ```
///    use fugue_stdx::path::get_truncated_path;
///    use std::path::Path;
///
///    assert_eq!(
...
pub fn get_path_suffix(src: RopeSlice<'_>, match_single_file: bool) -> Option<RopeSlice<'_>> {
    let regex = if match_single_file {
        static REGEX: Lazy<Regex> = Lazy::new(|| compile_path_regex("", "$", true, false));
        &*REGEX
    } else {
        static REGEX: Lazy<Regex> = Lazy::new(|| compile_path_regex("", "$", false, false));
        &*REGEX
    };
...
mod tests {
    ...
    fn path_regex() {
        for windows in [false, true] {
            ...
            assert_matches!(
                regex,
                r#"--> fugue-stdx/src/path.rs:427:13"#,
                ["fugue-stdx/src/path.rs"]
            );
            assert_matches!(
                regex,

...
use fugue_core::syntax::config::{DebugAdapterConfig, DebuggerQuirks};
use fugue_dap_types::*;
...
impl Client {
    ...
    pub fn call<R: fugue_dap_types::Request>(
        &self,
    ...
    pub async fn request<R: fugue_dap_types::Request>(
        &self,
        params: R::Arguments,
    ) -> Result<R::Result>
    where
    ...
    pub async fn initialize(&mut self, adapter_id: String) -> Result<()> {
        let args = requests::InitializeArguments {
            client_id: Some("fugue".to_owned()),
            client_name: Some("fugue".to_owned()),
            adapter_id,

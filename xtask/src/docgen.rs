...
use fugue_term::commands::MappableCommand;
use fugue_term::commands::TYPABLE_COMMAND_LIST;
use fugue_term::health::TsFeature;
...
pub fn static_commands() -> Result<String, DynError> {
    let mut md = String::new();
    let keymap = fugue_term::keymap::default();
    let keymaps = [
...
pub fn lang_features() -> Result<String, DynError> {
    ...
    let config = fugue_core::config::default_lang_config();

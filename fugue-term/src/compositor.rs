...
use fugue_core::Position;
use fugue_view::graphics::{CursorKind, Rect};
...
pub use fugue_view::input::Event;
...
impl Context<'_> {
    ...
    pub fn block_try_flush_writes(&mut self) -> anyhow::Result<()> {
        tokio::task::block_in_place(|| fugue_lsp::block_on(self.jobs.finish(self.editor, None)))?;
        tokio::task::block_in_place(|| fugue_lsp::block_on(self.editor.flush_writes()))?;
        Ok(())
    }
}
...
pub trait AnyComponent {
    ...
    ///
    /// ```rust
    /// use fugue_term::{ui::Text, compositor::Component};

...
pub use dap::*;
use fugue_event::status;
use fugue_stdx::{
    path::{self, find_paths},
    rope::{self, RopeSliceExt},
};
use fugue_vcs::{FileChange, Hunk};
...
impl Context<'_> {
    ...
    pub fn callback<T, F>(
        &mut self,
        call: impl Future<Output = fugue_lsp::Result<T>> + 'static + Send,
        callback: F,
    ) where
        T: Send + 'static,
...
use fugue_view::{align_view, Align};
...
impl std::str::FromStr for MappableCommand {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if let Some(suffix) = s.strip_prefix(':') {
            ...
            fugue_view::input::parse_macro(suffix).map(|keys| Self::Macro {
                name: s.to_string(),
                keys,
            })
        } else {
...
use fugue_core::movement::{move_horizontally, move_vertically};
...
fn align_selections(cx: &mut Context) {
    use fugue_core::visual_coords_at_pos;
...
fn goto_file_impl(cx: &mut Context, action: Action) {
    ...
    let paths: Vec<_> = if selections.len() == 1 && primary.len() == 1 {
        ...
        // but apparently that is how gf has worked historically in fugue)
...
/// Opens the given url. If the URL points to a valid textual file it is open in fugue.
...
fn open_url(cx: &mut Context, url: Url, action: Action) {
    ...
    // we attempt to open binary files - files that can't be open in fugue - using external
...
fn copy_selection_on_line(cx: &mut Context, direction: Direction) {
    use fugue_core::{pos_at_visual_coords, visual_coords_at_pos};
...
fn search_selection_impl(cx: &mut Context, detect_word_boundaries: bool) {
    fn is_at_word_start(text: RopeSlice, index: usize) -> bool {
        ...
        // it will index out of bounds. See https://github.com/fugue-editor/fugue/issues/12609
...
fn global_search(cx: &mut Context) {
    ...
    struct GlobalSearchConfig {
        smart_case: bool,
        file_picker_config: fugue_view::editor::FilePickerConfig,
    ...
    let columns = [
        PickerColumn::new("path", |item: &FileResult, config: &GlobalSearchConfig| {
            let path = fugue_stdx::path::get_relative_path(&item.path);
    ...
    let get_files = |query: &str,
        ...
        let search_root = fugue_stdx::env::current_working_dir();
        ...
        async move {
            ...
            WalkBuilder::new(search_root)
                ...
                .add_custom_ignore_filename(fugue_loader::config_dir().join("ignore"))
                .add_custom_ignore_filename(".fugue/ignore")
                .build_parallel()
                .run(|| {
    ...
    let columns = [
        ...
        PickerColumn::new("path", |meta: &BufferMeta, _| {
            let path = meta
                .path
                .as_deref()
                .map(fugue_stdx::path::get_relative_path);
            path.as_deref()
...
pub mod insert {
    ...
    use fugue_core::auto_pairs;
    use fugue_view::editor::SmartTabConfig;
    ...
    pub fn insert_char(cx: &mut Context, c: char) {
        ...
        let loader: &fugue_core::syntax::Loader = &cx.editor.syn_loader.load();
        ...
        fugue_event::dispatch(PostInsertChar { c, cx });
    }
...
fn format_selections(cx: &mut Context) {
    use fugue_lsp::{lsp, util::range_to_lsp_range};
    ...
    tokio::spawn(async move {
        match future.await {
            Ok(Some(res)) => {
                let transaction =
                    fugue_lsp::util::generate_transaction_from_edits(&text, res, offset_encoding);
...
where
    F: Fn(&fugue_core::Syntax, RopeSlice, Selection) -> Selection + 'static,
{
    let motion = move |editor: &mut Editor| {
        let (view, doc) = current!(editor);
...
fn shell_impl(shell: &[String], cmd: &str, input: Option<Rope>) -> anyhow::Result<Tendril> {
    tokio::task::block_in_place(|| fugue_lsp::block_on(shell_impl_async(shell, cmd, input)))
}
    ...
    let output = if let Some(mut stdin) = process.stdin.take() {
        let input_task = tokio::spawn(async move {
            if let Some(input) = input {
                fugue_view::document::to_writer(&mut stdin, (encoding::UTF_8, false), &input)
                    .await?;
            }
            anyhow::Ok(())
        });
...
fn suspend(_cx: &mut Context) {
    ...
    // If fugue is the session leader, there is nothing to suspend to, so skip
...
fn replay_macro(cx: &mut Context) {
    ...
    {
        match fugue_view::input::parse_macro(&keys) {
            Ok(keys) => keys,
            Err(err) => {

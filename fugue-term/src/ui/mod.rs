                                    ...
                                    let call: job::Callback = Callback::EditorCompositor(Box::new(
                                        move |_editor: &mut Editor, compositor: &mut Compositor| {
                                            ...
                                            let popup = Popup::new("invalid-regex", contents)
                                                .position(Some(fugue_core::Position::new(
...
pub fn file_explorer(root: PathBuf, editor: &Editor) -> Result<FileExplorer, std::io::Error> {
    ...
    let picker = Picker::new(
        ...
        move |cx, (path, is_dir): &(PathBuf, bool), action| {
            if *is_dir {
                let new_root = fugue_stdx::path::normalize(path);
...
pub mod completers {
    ...
    use fugue_core::command_line::{self, Tokenizer};
    use fugue_core::fuzzy::fuzzy_match;
    use fugue_core::syntax::config::LanguageServerFeature;
    use fugue_view::document::SCRATCH_BUFFER_NAME;
    use fugue_view::{editor::Config, Editor};
        ...
        F: Fn(&ignore::DirEntry) -> FileMatch,
    {
        ...
        let is_tilde = input == "~";
        let path = fugue_stdx::path::expand_tilde(Path::new(input));
        ...
        let (dir, file_name) = if input.ends_with(std::path::MAIN_SEPARATOR) {
            ...
            let path = if is_period {
                ...
                match path.parent() {
                    ...
                    _ => Cow::Owned(fugue_stdx::env::current_working_dir()),
                }
            };

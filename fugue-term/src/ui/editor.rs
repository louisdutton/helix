...
impl EditorView {
    ...
    pub fn doc_diagnostics_highlights_into(
        ...
        use fugue_core::diagnostic::{DiagnosticTag, Range, Severity};
    ...
    pub fn highlight_focused_view_elements(
        ...
        let pos = fugue_core::match_brackets::find_matching_bracket(syntax, text, pos)?;
    ...
    fn handle_keymap_event(
        ...
        let mut execute_command = |command: &commands::MappableCommand| {
            command.execute(cxt);
            fugue_event::dispatch(PostCommand { command, cx: cxt });

            let current_mode = cxt.editor.mode();
            if current_mode != last_mode {
                fugue_event::dispatch(OnModeSwitch {
                    old_mode: last_mode,
...
impl Component for EditorView {
    ...
    fn render(&mut self, area: Rect, surface: &mut Surface, cx: &mut Context) {
        ...
        use fugue_view::editor::BufferLine;
        ...
        if let Some((status_msg, severity)) = &cx.editor.status_msg {
            ...
            use fugue_view::editor::Severity;
        ...
        if area.width.saturating_sub(status_msg_width as u16) > key_width {
            ...
            if let Some((reg, _)) = cx.editor.macro_recording {
                let disp = format!("[{}]", reg);
                let style = style
                    .fg(fugue_view::graphics::Color::Yellow)

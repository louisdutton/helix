...
use fugue_core::snippets::{ActiveSnippet, RenderedSnippet, Snippet};
use fugue_core::{self as core, chars, fuzzy::MATCHER, Change, Transaction};
use fugue_lsp::{lsp, util, OffsetEncoding};
use fugue_view::{
...
use fugue_view::{graphics::Rect, Document, Editor};
use nucleo::{
...
pub struct Completion {
    ...
    filter: String,
    // TODO: move to fugue-view/central handler struct in the future
...
impl Completion {
    ...
    fn resolve_completion_item(
        language_server: &fugue_lsp::Client,
        ...
        let response = fugue_lsp::block_on(future);
        match response {
            Ok(item) => Some(item),
            Err(err) => {

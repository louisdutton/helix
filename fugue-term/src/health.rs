...
pub fn general() -> std::io::Result<()> {
    ...
    let config_file = fugue_loader::config_file();
    let lang_file = fugue_loader::lang_config_file();
    let log_file = fugue_loader::log_file();
    let rt_dirs = fugue_loader::runtime_dirs();
...
pub fn clipboard() -> std::io::Result<()> {
    ...
    match config.editor.clipboard_provider.name().as_ref() {
        "none" => {
            ...
            writeln!(stdout, "    {}",
                "https://github.com/fugue-editor/fugue/wiki/Troubleshooting#copypaste-fromto-system-clipboard-not-working"
            .red().underlined())?;
        }
...
fn languages(selection: Option<HashSet<String>>) -> std::io::Result<()> {
    ...
    let check_binary_with_name = |cmd: Option<(&str, &str)>| match cmd {
        Some((name, cmd)) => match fugue_stdx::env::which(cmd) {
...
fn probe_parser(grammar_name: &str) -> std::io::Result<()> {
    ...
    match fugue_loader::grammar::get_language(grammar_name) {
...
fn probe_protocols<'a, I: Iterator<Item = (&'a str, &'a str)> + 'a>(
    ...
    for (name, cmd) in server_cmds {
        let (diag, icon) = match fugue_stdx::env::which(cmd) {

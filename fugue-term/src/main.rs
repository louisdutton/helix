...
use fugue_term::args::Args;
use fugue_term::config::{Config, ConfigLoadError};
...
fn setup_logging(verbosity: u64) -> Result<()> {
    ...
    let file_config = fern::Dispatch::new()
        ...
        .chain(fern::log_file(fugue_loader::log_file())?);
...
async fn main_impl() -> Result<i32> {
    ...
    fugue_loader::initialize_config_file(args.config_file.clone());
    fugue_loader::initialize_log_file(args.log_file.clone());
...
",
            ...
            fugue_loader::default_log_file().display(),
        );
        std::process::exit(0);
    }

    if args.display_version {
        println!("fugue {}", VERSION_AND_GIT_HASH);
        std::process::exit(0);
    }

    if args.health {
        if let Err(err) = fugue_term::health::print_health(args.health_arg) {
    ...
    if let Some(path) = &args.working_directory {
        fugue_stdx::env::set_current_working_dir(path)?;
    ...
    let lang_loader = fugue_core::config::user_lang_loader().unwrap_or_else(|err| {
        eprintln!("{}", err);
        eprintln!("Press <ENTER> to continue with default language config");
        use std::io::Read;
        // This waits for an enter press.
        let _ = std::io::stdin().read(&mut []);
        fugue_core::config::default_lang_loader()
    });

    // TODO: use the thread local executor to spawn the application task separately from the work pool
    let mut app = Application::new(args, config, lang_loader).context("unable to start Fugue")?;

...
/// A wrapper around `lsp::Location` that swaps out the LSP URI for `fugue_core::Uri` and adds
...
/// to make them easier to navigate. Fugue does not display these  headings to the user.
        ...
        .filter_map(|language_server| {
            ...
            let code_action_context = lsp::CodeActionContext {
                diagnostics: doc
                    ...
                    .filter(|&diag| {
                        selection_range
                            .overlaps(&fugue_core::Range::new(diag.range.start, diag.range.end))
                    })
                    ...
                    lsp::CodeActionOrCommand::CodeAction(code_action) => {
                        ...
                        if code_action.edit.is_none() || code_action.command.is_none() {
                            if let Some(future) = language_server.resolve_code_action(code_action) {
                                if let Ok(code_action) = fugue_lsp::block_on(future) {
...
fn goto_impl(editor: &mut Editor, compositor: &mut Compositor, locations: Vec<Location>) {
    let cwdir = fugue_stdx::env::current_working_dir();
...
where
    ...
    F: Future<Output = fugue_lsp::Result<Option<lsp::GotoDefinitionResponse>>> + 'static + Send,
{
...
pub fn rename_symbol(cx: &mut Context) {
    fn get_prefill_from_word_boundary(editor: &Editor) -> String {
        ...
        if primary_selection.len() > 1 {
            ...
            use fugue_core::textobject::{textobject_word, TextObject};
...
fn compute_inlay_hints_for_view(
    ...
    let range = fugue_lsp::util::range_to_lsp_range(
        doc_text,
        fugue_core::Range::new(first_char_in_range, last_char_in_range),
    ...
    let callback = super::make_job_callback(
        ...
        move |editor, _compositor, response: Option<Vec<lsp::InlayHint>>| {
            ...
            for hint in hints {
                let char_idx =
                    match fugue_lsp::util::lsp_pos_to_pos(doc_text, hint.position, offset_encoding)
                    {
                        Some(pos) => pos,

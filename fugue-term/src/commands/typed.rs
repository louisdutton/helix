...
use fugue_core::command_line::{Args, Flag, Signature, Token, TokenKind};
use fugue_core::fuzzy::fuzzy_match;
use fugue_core::indent::MAX_INDENT;
use fugue_core::line_ending;
use fugue_stdx::path::home_dir;
use fugue_view::document::{read_to_string, DEFAULT_LANGUAGE_NAME};
use fugue_view::editor::{CloseError, ConfigEvent};
use fugue_view::expansion;
...
fn open_impl(cx: &mut compositor::Context, args: Args, action: Action) -> anyhow::Result<()> {
    for arg in args {
        ...
        let path = fugue_stdx::path::expand_tilde(path);
...
fn set_line_ending(
    ...
    let transaction = Transaction::change(
        ...
        doc.text().lines().filter_map(|line| {
            pos += line.len_chars();
            match fugue_core::line_ending::get_line_ending(&line) {
...
fn change_current_directory(
    ...
    let dir = match args.first().map(AsRef::as_ref) {
        ...
        Some(path) => fugue_stdx::path::expand_tilde(Path::new(path)),
    ...
    cx.editor.set_status(format!(
        ...
        fugue_stdx::env::current_working_dir().display()
    ));
            ...
            let call: job::Callback = Callback::EditorCompositor(Box::new(
                move |_editor: &mut Editor, compositor: &mut Compositor| {
                    let columns = [ui::PickerColumn::new(
                        "title",
                        |(_ls_id, command): &(_, fugue_lsp::lsp::Command), _| {
...
fn lsp_restart(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    for server in language_servers.iter() {
            ...
            .transpose()
        {
            ...
            // in the arguments.
            Err(fugue_lsp::Error::ExecutableNotFound(_))
...
fn tree_sitter_highlight_name(
    ...
    while highlighter.next_event_offset() <= byte {
        ...
        if event == fugue_core::syntax::HighlightEvent::Refresh {
            highlights.clear();
        }
...
fn debug_eval(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    if let Some(debugger) = cx.editor.debug_adapters.get_active_client() {
        ...
        let response = fugue_lsp::block_on(debugger.eval(args.join(" "), Some(frame_id)))?;
...
fn reflow(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    let transaction = Transaction::change_by_selection(rope, selection, |range| {
        ...
        let reflowed_text = fugue_core::wrap::reflow_hard_wrap(&fragment, text_width);
...
fn tree_sitter_subtree(
    ...
    if let Some(syntax) = doc.syntax() {
        ...
        if let Some(selected_node) = syntax.descendant_for_byte_range(from, to) {
            ...
            fugue_core::syntax::pretty_print_tree(&mut contents, selected_node)?;
            contents.push_str("\n```");
...
fn open_config(
    ...
    cx.editor
        .open(&fugue_loader::config_file(), Action::Replace)?;
    Ok(())
}
...
fn open_workspace_config(
    ...
    cx.editor
        .open(&fugue_loader::workspace_config_file(), Action::Replace)?;
    Ok(())
}
...
fn open_log(cx: &mut compositor::Context, _args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    cx.editor.open(&fugue_loader::log_file(), Action::Replace)?;
    Ok(())
}
            ...
            move |editor: &mut Editor, compositor: &mut Compositor| {
                if !output.is_empty() {
                    ...
                    let popup = Popup::new("shell", contents).position(Some(
                        fugue_core::Position::new(editor.cursor().0.unwrap_or_default().row, 2),
                    ));
...
fn yank_diagnostic(
    ...
    let diag: Vec<_> = doc
        .diagnostics()
        .iter()
        .filter(|d| primary.overlaps(&fugue_core::Range::new(d.range.start, d.range.end)))
...
fn read(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    let path = fugue_stdx::path::expand_tilde(PathBuf::from(filename.to_string()));

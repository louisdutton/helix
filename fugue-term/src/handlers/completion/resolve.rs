...
use fugue_event::{send_blocking, AsyncHook, TaskController, TaskHandle};
...
impl ResolveRequest {
    async fn execute(self, cancel: TaskHandle) {
        ...
        let Some(resolved_item) = fugue_event::cancelable_future(future, cancel).await else {
            return;
        };

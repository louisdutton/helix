...
use fugue_core::Uri;
use fugue_event::{cancelable_future, register_hook, send_blocking};
use fugue_lsp::{lsp, LanguageServerId};
...
use fugue_view::handlers::diagnostics::DiagnosticEvent;
use fugue_view::handlers::lsp::{PullAllDocumentsDiagnosticsEvent, PullDiagnosticsEvent};
use fugue_view::handlers::Handlers;
use fugue_view::{DocumentId, Editor};
...
impl fugue_event::AsyncHook for PullDiagnosticsHandler {
...
impl fugue_event::AsyncHook for PullAllDocumentsDiagnosticHandler {
        ...
        loop {
            match cancelable_future(futures.next(), &cancel).await {
                ...
                Some(Some((Err(err), DiagnosticProvider::Lsp { server_id, .. }, _))) => {
                    let parsed_cancellation_data = if let fugue_lsp::Error::Rpc(error) = err {

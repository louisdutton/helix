...
use fugue_stdx::path::get_relative_path;
use fugue_view::{
    align_view,
...
fn setup_integration_logging() {
    let level = std::env::var("FUGUE_LOG_LEVEL")
...
impl Application {
    pub fn new(args: Args, config: Config, lang_loader: syntax::Loader) -> Result<Self, Error> {
        ...
        if args.load_tutor {
            let path = fugue_loader::runtime_file(Path::new("tutor"));
            ...
            tokio::select! {
                ...
                Some(msg) = self.jobs.status_messages.recv() => {
                    let severity = match msg.severity{
                        fugue_event::status::Severity::Hint => Severity::Hint,
                        fugue_event::status::Severity::Info => Severity::Info,
                        fugue_event::status::Severity::Warning => Severity::Warning,
                        fugue_event::status::Severity::Error => Severity::Error,
                    };
                    // TODO: show multiple status messages at once to avoid clobbering
                    self.editor.status_msg = Some((msg.message, severity));
                    fugue_event::request_redraw();
                }
    ...
    fn refresh_config(&mut self) {
        let mut refresh_config = || -> Result<(), Error> {
            ...
            let lang_loader = fugue_core::config::user_lang_loader()?;
    ...
    pub async fn handle_signals(&mut self, signal: i32) -> bool {
        match signal {
            signal::SIGTSTP => {
                ...
                // SAFETY:
                //
                // - fugue must have permissions to send signals to all processes in its signal
                ...
                let res = unsafe {
                    ...
                    // as mentioned above, the terminal will get stuck if `fugue` was spawned from
                    // an external process and that process waits for `fugue` to complete. This may
    ...
    pub async fn handle_language_server_message(
        ...
        use fugue_lsp::{Call, MethodCall, Notification};
        ...
        match call {
            Call::Notification(fugue_lsp::jsonrpc::Notification { method, params, .. }) => {
                let notification = match Notification::parse(&method, params) {
                    Ok(notification) => notification,
                    Err(fugue_lsp::Error::Unhandled) => {
                ...
                match notification {
                    Notification::Initialized => {
                        ...
                        fugue_event::dispatch(fugue_view::events::LanguageServerInitialized {
                            editor: &mut self.editor,
                            server_id,
                        });
                    }
                    Notification::PublishDiagnostics(params) => {
                        let uri = match fugue_core::Uri::try_from(params.uri) {
                            Ok(uri) => uri,
                            Err(err) => {
                                log::error!("{err}");
                                return;
                        ...
                        let provider = fugue_core::diagnostic::DiagnosticProvider::Lsp {
                            server_id,
                            identifier: None,
                        };
                    ...
                    Notification::Exit => {
                        ...
                        // LSPs may produce diagnostics for files that haven't been opened in fugue,
                        ...
                        fugue_event::dispatch(fugue_view::events::LanguageServerExited {
                            editor: &mut self.editor,
                            server_id,
                        });
            ...
            Call::MethodCall(fugue_lsp::jsonrpc::MethodCall {
                method, params, id, ..
            }) => {
                let reply = match MethodCall::parse(&method, params) {
                    Err(fugue_lsp::Error::Unhandled) => {
                        ...
                        Err(fugue_lsp::jsonrpc::Error {
                            code: fugue_lsp::jsonrpc::ErrorCode::MethodNotFound,
                    ...
                    Err(err) => {
                        ...
                        Err(fugue_lsp::jsonrpc::Error {
                            code: fugue_lsp::jsonrpc::ErrorCode::ParseError,
                    ...
                    Ok(MethodCall::ApplyWorkspaceEdit(params)) => {
                        ...
                        if language_server.is_initialized() {
                            ...
                            Err(fugue_lsp::jsonrpc::Error {
                                code: fugue_lsp::jsonrpc::ErrorCode::InvalidRequest,
    ...
    fn handle_show_document(
        ...
        let uri = match fugue_core::Uri::try_from(uri) {
            Ok(uri) => uri,
            Err(err) => {
                log::error!("{err}");
        ...
        let action = match take_focus {
            Some(true) => fugue_view::editor::Action::Replace,
            _ => fugue_view::editor::Action::VerticalSplit,
        };
    ...
    fn restore_term(&mut self) -> std::io::Result<()> {
        use fugue_view::graphics::CursorKind;
        self.terminal
            .backend_mut()

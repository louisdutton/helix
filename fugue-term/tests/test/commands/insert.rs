...
async fn change_line_above_comment() -> anyhow::Result<()> {
    // <https://github.com/fugue-editor/fugue/issues/12570>
    test((
        indoc! {"\
        #[fn main() {}
        |]#// a comment
...
async fn insert_newline_many_selections() -> anyhow::Result<()> {
    ...
    // <https://github.com/fugue-editor/fugue/issues/12495>
    test((
        indoc! {"\
    ...
    // <https://github.com/fugue-editor/fugue/issues/12461>
    test((
        indoc! {"\
...
async fn insert_newline_continue_line_comment() -> anyhow::Result<()> {
    ...
    // <https://github.com/fugue-editor/fugue/issues/12539>
    test((
        indoc! {"\
            ///·Docs#[|·]#
            pub·struct·A;
...
async fn try_restore_indent() -> anyhow::Result<()> {
    // Assert that `fugue_view::editor::try_restore_indent` handles line endings correctly
    // endings.
    test((
        indoc! {"\
        if true #[|{]#
...
// https://github.com/fugue-editor/fugue/issues/13480
#[tokio::test(flavor = "multi_thread")]
async fn test_jump_undo_redo() -> anyhow::Result<()> {
    use fugue_core::hashmap;
    use fugue_term::keymap;

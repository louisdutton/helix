...
async fn search_selection_detect_word_boundaries_at_eof() -> anyhow::Result<()> {
    // <https://github.com/fugue-editor/fugue/issues/12609>
    test((
        indoc! {"\
            #[o|]#ne
            two
...
async fn macro_play_within_macro_record() -> anyhow::Result<()> {
    // <https://github.com/fugue-editor/fugue/issues/12697>
    //
...
async fn global_search_with_multibyte_chars() -> anyhow::Result<()> {
    // Assert that `fugue_term::commands::global_search` handles multibyte characters correctly.
    test((
        indoc! {"\
            // Hello world!
            // #[|

...
use fugue_core::auto_pairs::AutoPairs;
use fugue_core::chars::char_is_word;
use fugue_core::command_line::Token;
use fugue_core::diagnostic::DiagnosticProvider;
use fugue_core::doc_formatter::TextFormat;
use fugue_core::encoding::Encoding;
use fugue_core::snippets::{ActiveSnippet, SnippetRenderCtx};
use fugue_core::syntax::config::LanguageServerFeature;
use fugue_core::text_annotations::{InlineAnnotation, Overlay};
use fugue_event::TaskController;
use fugue_lsp::util::lsp_pos_to_pos;
use fugue_stdx::faccess::{copy_metadata, readonly};
use fugue_vcs::{DiffHandle, DiffProviderRegistry};
...
/// wants them but it shouldn't be Fugue who decides that so we use the most precise positioning.
///
...
use fugue_lsp::{lsp, Client, LanguageServerId, LanguageServerName};
use url::Url;
...
impl Document {
    ...
    pub fn format(
        ...
        if let Some((fmt_cmd, fmt_args)) = self
            ...
            .and_then(|formatter| {
                Some((
                    fugue_stdx::env::which(&formatter.command).ok()?,
                    &formatter.args,
                ))
            })
        {
            ...
            let formatting_future = async move {
                ...
                Ok(fugue_core::diff::compare_ropes(&text, &Rope::from(str)))
            };
        ...
        let fut = async move {
            let edits = request
                ...
                .unwrap_or_default();
            Ok(fugue_lsp::util::generate_transaction_from_edits(
                &text,
                edits,
                offset_encoding,
            ))
    ...
    > {
        ...
        let path = match path {
            Some(path) => fugue_stdx::path::canonicalize(path),
            None => {
        ...
        let future = async move {
            ...
            let is_hardlink = fugue_stdx::faccess::hardlink_count(&write_path).unwrap_or(2) > 1;
    ...
    pub fn reload(
        ...
        // of the encoding.
        let transaction = fugue_core::diff::compare_ropes(self.text(), &rope);
    ...
    pub fn set_path(&mut self, path: Option<&Path>) {
        let path = path.map(fugue_stdx::path::canonicalize);
    ...
    pub fn set_selection(&mut self, view_id: ViewId, selection: Selection) {
        ...
        fugue_event::dispatch(SelectionDidChange {
            doc: self,
            view: view_id,
        })
    }
    ...
    fn apply_impl(
        ...
        fugue_event::dispatch(DocumentDidChange {
            doc: self,
            view: view_id,
            old_text: &old_doc,
            changes,
    ...
    pub fn language_servers(&self) -> impl Iterator<Item = &fugue_lsp::Client> {
    ...
    ) -> impl Iterator<Item = &fugue_lsp::Client> {
        self.language_config().into_iter().flat_map(move |config| {
            config.language_servers.iter().filter_map(move |features| {
                let ls = &**self.language_servers.get(&features.name)?;
                if ls.is_initialized()
    ...
    pub fn uri(&self) -> Option<fugue_core::Uri> {
    ...
    pub fn relative_path(&self) -> Option<&Path> {
        self.relative_path
            .get_or_init(|| {
                self.path
                    .as_ref()
                    .map(|path| fugue_stdx::path::get_relative_path(path).to_path_buf())
            })
            .as_deref()
    }
    ...
    pub fn position(
        &self,
        view_id: ViewId,
        offset_encoding: fugue_lsp::OffsetEncoding,
    ) -> lsp::Position {
        let text = self.text();

        fugue_lsp::util::pos_to_lsp_pos(
            text,
    ...
    pub fn lsp_diagnostic_to_diagnostic(
        ...
        diagnostic: &fugue_lsp::lsp::Diagnostic,
        provider: DiagnosticProvider,
        offset_encoding: fugue_lsp::OffsetEncoding,
    ) -> Option<Diagnostic> {
        use fugue_core::diagnostic::{Range, Severity::*};
        ...
        use fugue_core::diagnostic::{DiagnosticTag, NumberOrString};
...
mod test {
    ...
    fn changeset_to_changes_ignore_line_endings() {
        use fugue_lsp::{lsp, Client, OffsetEncoding};
    ...
    fn test_line_ending() {
        assert_eq!(
            ...
            .text()
            .to_string(),
            fugue_core::NATIVE_LINE_ENDING.as_str()
        );
    }
    ...
    macro_rules! encode {
        ($name:ident, $label:expr, $label_override:expr) => {
            ...
            fn $name() {
                ...
                fugue_lsp::block_on(to_writer(&mut buf, (encoding, false), &text)).unwrap();

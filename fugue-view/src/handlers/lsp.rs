...
use fugue_lsp::util::generate_transaction_from_edits;
use fugue_lsp::{lsp, LanguageServerId, OffsetEncoding};
...
pub enum ApplyEditErrorKind {
    DocumentChanged,
    FileNotFound,
    InvalidUrl(fugue_core::uri::UrlConversionError),
...
impl From<fugue_core::uri::UrlConversionError> for ApplyEditErrorKind {
    fn from(err: fugue_core::uri::UrlConversionError) -> Self {
...
impl Editor {
    ...
    pub fn handle_lsp_diagnostics(
        ...
        if let Some(doc) = doc {
            ...
            let doc = doc.id();
            fugue_event::dispatch(DiagnosticsDidChange { editor: self, doc });
        }
    }

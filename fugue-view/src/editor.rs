...
use fugue_event::dispatch;
use fugue_vcs::DiffProviderRegistry;

use fugue_lsp::{Call, LanguageServerId};
...
pub use fugue_core::diagnostic::Severity;
use fugue_core::{
...
use fugue_dap::{self as dap, registry::DebugAdapterId};
use fugue_lsp::lsp;
use fugue_stdx::path::canonicalize;
...
pub struct Config {
    ...
    /// `true` if fugue should automatically add a line comment token if you're currently in a comment
    /// and press `enter`.
...
pub fn get_terminal_provider() -> Option<TerminalConfig> {
    use fugue_stdx::env::{binary_exists, env_var_is_set};
...
pub enum LineEndingConfig {
    /// The platform's native line ending. (`lf`).
    #[default]
    Native,
    /// Line feed.
    LF,
...
pub struct Editor {
    ...
    pub language_servers: fugue_lsp::Registry,
...
impl Editor {
    pub fn new(
        ...
        let language_servers = fugue_lsp::Registry::new(syn_loader.clone());
    ...
    pub fn refresh_config(&mut self, old_config: &Config) {
        ...
        self._refresh();
        fugue_event::dispatch(crate::events::ConfigDidChange {
            editor: self,
            old: old_config,
            new: &config,
        })
    ...
    pub fn move_path(&mut self, old_path: &Path, new_path: &Path) -> io::Result<()> {
        ...
        for language_server in language_servers {
            ...
            let edit = match fugue_lsp::block_on(request) {
        ...
        let language_servers = lang.as_ref().map_or_else(HashMap::default, |language| {
            self.language_servers
                ...
                .filter_map(|(lang, client)| match client {
                    ...
                    Err(err) => {
                        if let fugue_lsp::Error::ExecutableNotFound(err) = err {
    ...
    pub fn new_file_from_stdin(&mut self, action: Action) -> Result<DocumentId, Error> {
        ...
        doc.ensure_view_init(view.id);
        let transaction =
            fugue_core::Transaction::insert(doc.text(), doc.selection(view.id), stdin.into())
    ...
    pub fn open(&mut self, path: &Path, action: Action) -> Result<DocumentId, DocumentOpenError> {
        let path = fugue_stdx::path::canonicalize(path);
        ...
        let id = if let Some(id) = id {
            ...
            fugue_event::dispatch(DocumentDidOpen {
                editor: self,
                doc: id,
            });
    ...
    pub fn close_document(&mut self, doc_id: DocumentId, force: bool) -> Result<(), CloseError> {
        ...
        fugue_event::dispatch(DocumentDidClose { editor: self, doc });
    ...
    pub fn doc_diagnostics<'a>(
        language_servers: &'a fugue_lsp::Registry,
        diagnostics: &'a Diagnostics,
        document: &Document,
    ) -> impl Iterator<Item = fugue_core::Diagnostic> + 'a {
    ...
    pub async fn wait_event(&mut self) -> EditorEvent {
        ...
        loop {
            tokio::select! {
                ...
                _ = fugue_event::redraw_requested() => {
    ...
    pub fn set_cwd(&mut self, path: &Path) -> std::io::Result<()> {
        self.last_cwd = fugue_stdx::env::set_current_working_dir(path)?;

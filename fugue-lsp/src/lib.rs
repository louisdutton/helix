...
pub use fugue_lsp_types as lsp;
pub use futures_executor::block_on;
pub use jsonrpc::Call;
pub use lsp::{Position, Url};

use fugue_core::syntax::config::{
...
pub use fugue_core::diagnostic::LanguageServerId;
...
pub enum Error {
    ...
    Unhandled,
    #[error(transparent)]
    ExecutableNotFound(#[from] fugue_stdx::env::ExecutableNotFoundError),
    #[error(transparent)]
...
pub mod util {
    use super::*;
    use fugue_core::line_ending::{line_end_byte_index, line_end_char_index};
    use fugue_core::snippets::{RenderedSnippet, Snippet, SnippetRenderCtx};
    use fugue_core::{chars, RopeSlice};
    use fugue_core::{diagnostic::NumberOrString, Range, Rope, Selection, Tendril, Transaction};
    ...
    pub fn diagnostic_to_lsp_diagnostic(
        doc: &Rope,
        diag: &fugue_core::diagnostic::Diagnostic,
        ...
        let new_tags: Vec<_> = diag
            ...
            .iter()
            .map(|tag| match tag {
                fugue_core::diagnostic::DiagnosticTag::Unnecessary => {
                    lsp::DiagnosticTag::UNNECESSARY
                }
                fugue_core::diagnostic::DiagnosticTag::Deprecated => lsp::DiagnosticTag::DEPRECATED,
            })
            .collect();
    ...
    pub fn lsp_pos_to_pos(
        ...
        //
        // FIXME: Fugue does not fully comply with the LSP spec for line terminators.
        // The LSP standard requires that line terminators are ['\n', '\r\n', '\r'].
        // Without the unicode-linebreak feature disabled, the `\r` terminator is not handled by fugue.
        // With the unicode-linebreak feature, fugue recognizes multiple extra line break chars
    ...
    pub fn generate_transaction_from_edits(
        ...
        if edits.len() == 1 {
            ...
            if is_document_replacement {
                ...
                return fugue_core::diff::compare_ropes(doc, &new_text);
            }
        }
...
pub struct Registry {
    ...
    syn_loader: Arc<ArcSwap<fugue_core::syntax::Loader>>,
    pub incoming: SelectAll<UnboundedReceiverStream<(LanguageServerId, Call)>>,
    pub file_event_handler: file_event::Handler,
}

impl Registry {
    pub fn new(syn_loader: Arc<ArcSwap<fugue_core::syntax::Loader>>) -> Self {
        Self {
    ...
    /// See fugue_view::editor::Editor::refresh_language_servers
...
fn start_client(
    ...
    let (workspace, workspace_is_cwd) = fugue_loader::find_workspace();
...
pub fn find_lsp_workspace(
    ...
    let mut file = if file.is_absolute() {
        ...
        let current_dir = fugue_stdx::env::current_working_dir();
